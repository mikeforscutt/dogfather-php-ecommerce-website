...
class ProductsList
{
    ...
    public function init()
    {
        ...
        if (isset($this->_mCategoryId)) {
         ...
         the GetProductsOnDepartment() business tier method */ elseif (
            isset($this->_mDepartmentId)
        ) {
...
tier method */ else {
        ...
        for ($i = 0; $i < count($this->mProducts); $i++) {
            $this->mProducts[$i]["link_to_product"] = Link::ToProduct(
                $this->mProducts[$i]["product_id"]
            );
            if ($this->mProducts[$i]["thumbnail"]) {
                $this->mProducts[$i]["thumbnail"] = Link::Build(
                    "product_images/" . $this->mProducts[$i]["thumbnail"]
                );
            }
        }
    }
