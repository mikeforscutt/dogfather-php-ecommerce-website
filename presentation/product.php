...
class Product
{
    ...
    public function init()
    {
        ...
        if (isset($_SESSION["link_to_continue_shopping"])) {
            $continue_shopping = Link::QueryStringToArray(
                $_SESSION["link_to_continue_shopping"]
            );
            $page = 1;
        ...
        if ($this->mProduct["image"]) {
            $this->mProduct["image"] = Link::Build(
                "product_images/" . $this->mProduct["image"]
            );
        }
        if ($this->mProduct["image_2"]) {
            $this->mProduct["image_2"] = Link::Build(
                "product_images/" . $this->mProduct["image_2"]
            );
        }
        ...
        for ($i = 0; $i < count($this->mLocations); $i++) {
            $this->mLocations[$i]["link_to_department"] = Link::ToDepartment(
                $this->mLocations[$i]["department_id"]
            );
            $this->mLocations[$i]["link_to_category"] = Link::ToCategory(
                $this->mLocations[$i]["department_id"],
